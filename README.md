...
- 使用預先定義的提示在配置文件中
  ...
  - 可以使用預定義功能或自定義功能來分類提示。內建多種的 [預定義系統函數](#預定義系統函數)。
  - 支援 GPT API 的請求參數修改
...
- 範本：必填
  ...
  - 要使用系統變數，請加上 `{system.*variableName*}`，variableName 可以是 [預定義系統變數](#預定義系統變數) 之一。
...
- 回應處理程序：可選 
  ...
  - 回應處理程序用於處理回應。預設使用取代函數。處理函數可以是 [預定義系統函數](#預定義系統函數) 或使用者定義函數之一。
  - 您可以使用以下方式設置回應處理程序：
